...
const sourceLabel: Record<string, string> = {
  INDIVIDU: 'Individu', KOMUNITAS: 'Komunitas', PERUSAHAAN: 'Perusahaan',
}
...
const downloadUrl = computed(() => {
  ...
  if (!url) return null
  // Untuk raw (PDF), langsung pakai URL asli via download attribute
  if (isPdf.value) return url
  // Untuk image, inject fl_attachment di Cloudinary URL
...
async function confirmDelete() {
  ...
  try {
    await store.deleteIncomeTransaction(id.value)
    router.push('/income-transactions')
  } catch {
    ...
    <main class="content">
      ...
      <template v-else-if="!store.currentItem">
        <div class="not-found">
          ...
          <button type="button" class="btn-primary" @click="router.push('/income-transactions')">
            Kembali ke Daftar
          </button>
        </div>
      </template>
        ...
        <div class="content-inner">
          ...
          <section class="card">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-row">
                ...
                <span class="detail-value">{{ store.currentItem.donorName || '—' }}</span>
              </div>
              <div class="detail-row">
          ...
          <section class="card">
            ...
            <template v-if="proofUrl">
              ...
              <!-- Download button (hanya untuk non-PDF, PDF sudah punya tombol Buka PDF) -->
          ...
          <!-- Back button -->
          <div class="back-row">
            <button type="button" class="btn-outline-gray" @click="router.push('/income-transactions')">
...
.status-verified   { background: #e6faf7; color: #00c6ac; }
.status-pending    { background: #FFF8E1; color: #F5A623; }
.status-rejected   { background: #fff1f2; color: #FF303E; }
...
.btn-outline-gray:disabled { opacity: 0.5; cursor: not-allowed; }
